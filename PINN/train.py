#!/usr/bin/env python3
"""
Unified training entrypoint for the PINN Solar System emulator.

Combines the functionality of PINN_train.py (single-stage) and PINN_train_improved.py 
(multi-stage with checkpointing). Supports both modes via --training-mode flag.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Unified training script for Solar System PINN",
...
def save_unified_figures(plots_dir: Path, artifacts: dict[str, Any]) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("RMSE")
    plt.title("PINN validation RMSE")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
