...
try:  # pragma: no cover - optional local logging helpers
    from .logging.distributed_logger import DistributedLogger
    from .logging.logging_utils import log_config_to_tensorboard
except Exception:  # pragma: no cover
    DistributedLogger = None

    def log_config_to_tensorboard(*args: Any, **kwargs: Any) -> None:
        return None
##
...
def _unwrap_model(model: EmulatorModel | DDP) -> EmulatorModel:
    """Return the wrapped emulator model when training under DDP."""
...
def _position_only_hybrid_to_full_state_norm(
    model: EmulatorModel | DDP,
    t_norm: Tensor,
    scaler_mean_t: Tensor,
    scaler_std_t: Tensor,
    time_std_seconds: float,
    ...
    """
    Evaluate a position-only hybrid model and expose correction components.
    ...
    - vel_phys: [N,B,3] km/s or None
    - acc_phys: [N,B,3] km/s^2 or None
    - base_pos_norm: [N,B,3] normalized base trajectory
    - correction_norm: [N,B,3] normalized correction trajectory
    - correction_gains: [B,1] learned positive correction gains
    """
    core_model = _unwrap_model(model)
    if core_model.state_mode != "position_only":
        raise ValueError("_position_only_hybrid_to_full_state_norm requires state_mode='position_only'")
    ...
    pos_norm, base_pos_norm, correction_norm = core_model.forward_components(t_norm)
    mean_pos = scaler_mean_t[:, :, :3]
    std_pos = scaler_std_t[:, :, :3]
    mean_vel = scaler_mean_t[:, :, 3:]
    std_vel = scaler_std_t[:, :, 3:]
    ...
    pred_norm_full = torch.cat((pos_norm, vel_norm), dim=-1)
    correction_gains = (
        core_model.correction_gains()
        if getattr(core_model, "hybrid_correction", False)
        else torch.zeros((core_model.num_bodies, 1), device=pos_norm.device, dtype=pos_norm.dtype)
    )
    return pred_norm_full, pos_phys, vel_phys, acc_phys, base_pos_norm, correction_norm, correction_gains


def _hybrid_correction_regularization_loss(
    correction_norm: Tensor,
    correction_gains: Tensor,
) -> tuple[Tensor, Tensor, Tensor]:
    """
    Keep the hybrid branch residual by penalizing:
    - normalized correction amplitude
    - learned correction gains
    """
    amplitude_loss = torch.mean(correction_norm * correction_norm)
    gain_loss = torch.mean(correction_gains * correction_gains)
    total = amplitude_loss + 0.1 * gain_loss
    return total, amplitude_loss, gain_loss
...
def train_emulator(
    ...
    if mcfg.num_bodies != len(bodies):
        mcfg = ModelConfig(
            ...
            hybrid_correction=mcfg.hybrid_correction,
            correction_layers=mcfg.correction_layers,
            correction_hidden_dim=mcfg.correction_hidden_dim,
            correction_init_scale=mcfg.correction_init_scale,
    ...
    for epoch_idx in epoch_iter:
        ...
        if cfg.correction_loss_weight > 0.0:
            corr_warmup = max(1, int(cfg.correction_warmup_epochs))
            start = max(0, int(cfg.correction_start_epoch))
            progress = max(0.0, float(epoch_idx + 1 - start)) / float(corr_warmup)
            correction_weight_eff = cfg.correction_loss_weight * min(1.0, progress)
        else:
            correction_weight_eff = 0.0
        ...
        running_correction_amp = 0.0
        running_correction_gain = 0.0
        n_batches = 0
        ...
        for batch_idx, (batch_t, batch_y) in enumerate(train_loader):
            ...
            if model.state_mode == "position_only":
                core_model = _unwrap_model(model)
                use_hybrid_correction = getattr(core_model, "hybrid_correction", False)
                ...
                batch_t_req = batch_t.detach().clone().requires_grad_(True)
                if use_hybrid_correction:
                    pred, pos_phys, vel_phys, acc_phys, _, correction_norm_batch, correction_gains = _position_only_hybrid_to_full_state_norm(
                        model=model,
                        t_norm=batch_t_req,
                    ...
                    correction_loss, correction_amp_loss, correction_gain_loss = _hybrid_correction_regularization_loss(
                        correction_norm=correction_norm_batch,
                    ...
                    correction_amp_loss = torch.zeros((), device=device)
                    correction_gain_loss = torch.zeros((), device=device)
                ...
                if use_collocation:
                    ...
                    if use_hybrid_correction:
                        _, pos_phys_coll, vel_phys_coll, acc_phys_coll, _, correction_norm_coll, correction_gains_coll = _position_only_hybrid_to_full_state_norm(
                            model=model,
                            t_norm=coll_t,
                            scaler_mean_t=scaler_mean_t,
                            scaler_std_t=scaler_std_t,
                        ...
                        coll_correction_loss, coll_correction_amp_loss, coll_correction_gain_loss = _hybrid_correction_regularization_loss(
                            correction_norm=correction_norm_coll,
                            correction_gains=correction_gains_coll,
                        )
                        correction_loss = 0.5 * (correction_loss + coll_correction_loss)
                        correction_amp_loss = 0.5 * (correction_amp_loss + coll_correction_amp_loss)
                        correction_gain_loss = 0.5 * (correction_gain_loss + coll_correction_gain_loss)
                    else:
            ...
            loss = (
                ...
                + correction_weight_eff * correction_loss
            )
        ...
        running_correction += float(correction_loss.detach().cpu().item())
        running_correction_amp += float(correction_amp_loss.detach().cpu().item())
        running_correction_gain += float(correction_gain_loss.detach().cpu().item())
        n_batches += 1
        ...
        if is_distributed:
            ...
            metrics_tensor = torch.tensor(
                [
                    ...
                    running_angular,
                    running_correction,
                    running_correction_amp,
                    running_correction_gain,
                    float(n_batches),
                ],
                dtype=torch.float32,
                device=device,
        ...
        correction_loss = running_correction / max(1, n_batches)
        correction_amp_loss = running_correction_amp / max(1, n_batches)
        correction_gain_loss = running_correction_gain / max(1, n_batches)
        ...
        if rank == 0 and writer is not None:
            ...
            writer.add_scalar('Hybrid/correction_loss', correction_loss, epoch_idx)
            writer.add_scalar('Hybrid/correction_amplitude_loss', correction_amp_loss, epoch_idx)
            writer.add_scalar('Hybrid/correction_gain_loss', correction_gain_loss, epoch_idx)
            writer.add_scalar('Hybrid/correction_weight', correction_weight_eff, epoch_idx)
        ...
        if rank == 0 and use_wandb:
            metrics = {
                ...
                'hybrid/correction_loss': correction_loss,
                ...
                'weights/correction': correction_weight_eff,
        ...
        history["correction_loss"].append(correction_loss)
        history["correction_amplitude_loss"].append(correction_amp_loss)
        history["correction_gain_loss"].append(correction_gain_loss)
        ...
        history["correction_weight"].append(float(correction_weight_eff))
        ...
        if progress_enabled:
            if model.state_mode == "position_only":
                postfix = {
                    ...
                    "corr": f"{correction_loss:.3e}",
                    "corr_w": f"{correction_weight_eff:.2e}",
            ...
            if model.state_mode == "position_only":
                msg = (
                    ...
                    f"corr={correction_loss:.3e} nbody_w={nbody_weight_eff:.2e} "
                    f"corr_w={correction_weight_eff:.2e} lr={optimizer.param_groups[0]['lr']:.2e}"
                )
                if np.isfinite(val_vel_rmse):
